...
{
	int wn =200; double dlt = 1./wn; double sum = 0, sumw =0;
   for(int i = -wn; i<= wn; i++ )
   {
...
{
if(vi == 0 ) return 0;
double sq2_ = sqrt(2.);  double mPi_ = asin(1.)*2;
...
{
if(fabs(ksi)<0.0001) ksi = 0.0001;
double sq2_ = 1.41421356;  double mPi_ = 1.77245385;
...
{
sq2 = sqrt(2.);  mPi = asin(1.)*2;
fi_erf_q =100; double dlt = 3./fi_erf_q;
...
{
double ret;
double ksin = ksi*fi_erf_q/sigc; if(ksin>3*(fi_erf_q-1)) ksin =3*(fi_erf_q-1);
...
inline EDP::REAL  EDP:: UpdMsgL1_(int * xr, EDP::REAL* M, EDP::REAL* Di_hat, EDP::REAL* add,int K, EDP::REAL lambda, EDP::REAL smoothMax)
{
    ...
    for (k=0; k<K; k++)
	{
     ...
     for (k=0; k<K; k++) TRUNCATE(M[xr[k]],  smoothMax);
	add[0] = (M[xr[K-1]]+ lambda< smoothMax) ? M[xr[K-1]]+ lambda :smoothMax;
    return delta;
}
...
{
 if (x > 0) addMsgC( &MsgC(x-1,y,0),  &MrgC(x,y), m_nLabels); // message (x-1,y)->(x,y)
 if (y > 0)  addMsgC( &MsgC(x,y-1,1),  &MrgC(x,y), m_nLabels);// message (x,y-1)->(x,y)
 if (x < m_width-1)  addMsgC( &MsgC(x+1,y,2),  &MrgC(x,y), m_nLabels);   // message (x+1,y)->(x,y)
 if (y < m_height-1) addMsgC( &MsgC(x,y+1,3),  &MrgC(x,y), m_nLabels);// message (x,y+1)->(x,y)
}

inline void  EDP::addMsg_(int * xr, int * xsh,EDP::REAL* M, EDP::REAL* Mrg,int Km,EDP::REAL add)
{

    for (int k=0; k<Km; k++)Mrg[xr[k]] += M[xsh[k]];
	Mrg[xr[Km]] += add;
...
inline void  EDP::subMsgC(EDP::REAL* Di, EDP::REAL* M, EDP::REAL* Mrg,int K, EDP::REAL gamma)
{
    ...
    for (int k=0; k<K; k++)Di[k]= gamma*Mrg[k] - M[k];
...
void EDP::initializeAlg()
{
    ...
    for(int c =0; c <3; c++) mean[c] += I_ims[0][p + c*m_nPixels];
	 for(int c =0; c <3; c++) mean[c] /= m_nPixels;
	 sigma_img =0; for(int p = 0; p < m_nPixels; p++){  float sum =0; for(int c =0; c <3; c++){ float vl = I_ims[0][p +c*m_nPixels]- mean[c]; sum += vl*vl;} sigma_img +=sum; }
...
{ /////////////////// COST CLC
		int Sqrt_d =0; int br_tms =1;  chTr = 30;
	     float * c_D = new float [m_height*Tau];
		  float avmin =0;
		 if(0) {computeDSI_tau( I_ims[0],  I_ims[1],  br_tms,  Sqrt_d, chTr);
	      avmin   = min_mean();
		 memcpy(c_D, m_D, m_height*Tau*sizeof(float));}
		 for(int i =0; i < m_height*Tau; i++) m_D[i]=0;
         Cost_mean = computeDSI_tauG( IG_ims[0],  IG_ims[1],  0,  Sqrt_d, chTr*4, 1);
		 float avmingr  = min_mean();  float gmul  = avmin/avmingr*3.5;
		 if(0)for(int i =0; i < m_height*Tau; i++)m_D[i] = c_D[i] + gmul*m_D[i] ;
		 float thr = 0.00095;
		 transform_D_( thr);
		 delete [] c_D;
}///////////////////////////////
	...
	int itr = (N_LB<30) ? 5:8;
	MRF__z( itr, m_answer, DspM);
	for(int p = 0 ; p < m_nPixels; p++) dsp[p] = 0.25* m_answer[p];
    delete [] DspM;
}
//void EDP::findSlt(float * dsp, char * c1, char * c2, float scl)
//{
//
//	int* DspM = new int [m_nPixels];
//	m_answer = new int [m_nPixels];
...
//	     float avmin   = min_mean();
//         memcpy(c_D, m_D, m_height*Tau*sizeof(float)); for(int i =0; i < m_height*Tau; i++) m_D[i]=0;
//         Cost_mean = computeDSI_tauG( IG_ims[0],  IG_ims[1],  0,  Sqrt_d, chTr*4, 1);
//		 float avmingr  = min_mean();  float gmul  = avmin/avmingr*3.5;
//		 for(int i =0; i < m_height*Tau; i++)m_D[i] = c_D[i] + gmul*m_D[i] ;
//		 float thr = 0.00095;
//		 transform_D_( thr);
//		 delete [] c_D;
//}///////////////////////////////
...
//	{
//    int p = x + y*m_width;
//	int xr = x - dsp[p]; int pr = (xr<0) ? y*m_width : xr + y*m_width;
//	int cor = c2[pr]-c1[p]; float crc = scl*cor;
//	dsp[p] += crc; if(dsp[p] < 0) dsp[p] = 0;
//	}
//    delete [] DspM;
//}
...
////////////////////INI filter /////////////////////////
 ...
 int cnt_big = 0; for(int p = 0 ; p < size1; p++) cnt_big += cnt_p[p];
   ...
   for(int i = 0 ; i <cnt_big*nb_ch; i++)sum_vl[i] =  0;
...
{
double  *CC = new double [m_nPixels*m_nLabels]; memcpy(CC, C, sizeof(double)*m_nPixels*m_nLabels);
...
for(int p =0; p < m_nPixels*m_nLabels; p++ ) CC[p] = C[p] - MM[p]+M[p];
}//----------------------------------------------------------------
...
for (int c =0; c< 3; c++) K_cls[0].cl[c] = 0;
for(int p = 0; p < m_nPixels; p++)
{
for (int c =0; c< 3; c++) K_cls[0].cl[c]  += im[c][p];
K_ind[p] = 0;
}
for (int c =0; c< 3; c++) K_cls[0].cl[c]  /= m_nPixels;
}///////////////////END CL =1 ///////////////////
{/////////////// CLUSTERS > 1 /////////////////
  unsigned int exp2[32]; exp2[0]=1; for(int i = 1; i < 32; i++) exp2[i] = exp2[i-1]*2;
  int lg2Q = 0; while(exp2[lg2Q] < cl_Q  ) lg2Q++;
  cl_Q = exp2[lg2Q];
...
//----------------------------- ini K_ind --------------
 for(int p = 0; p < m_nPixels; p++) K_ind[p] = 0;
...
{
	Cnt[ll] /= 2;  int sum =0; int md =0; while(Cnt[ll] > sum)sum += H[(md++) +  ll*256]; md--;
	Thr[ll] = md;
...
{//............ dvd into 2 .....
	int q = K_ind[p];
	if(imch[p] == Thr[q] && Cnt[q] == 0)K_ind[p] += lB;
	if(imch[p] == Thr[q] && Cnt[q]) Cnt[q]--;
    if(imch[p] >   Thr[q] ) K_ind[p] += lB;
}//..................................
 }//====================================
 for (int q =0; q< cl_Q; q++){Cnt[q] =0; for (int c =0; c< 3; c++) K_cls[q].cl[c] = 0;}
for(int p = 0; p < m_nPixels; p++)
{
	int q = K_ind[p]; Cnt[q]++;
	for (int c =0; c< 3; c++) K_cls[q].cl[c]  += im[c][p];
...
void EDP::MRF__(int itr, int * dL, int * dR)
{
	...
	//delete [] m_cost;
	Make_Gr_inf(0, 3,12); Smthnss[0].max = Lmbd_trn*Cost_mean; Smthnss[0].semi  =  Lmbd_trn*Cost_mean/4;
    TRWS_L(itr,Mrgnl, dL);
	//TRWS_R(itr,Mrgnl, dR);
	//
	//for(int i =0; i<3; i++){
...
void EDP::MRF__z(int itr, int * dL, int * dR)
{
	...
	Size_fw = 18;
	float sigma_c_mul =   0.75/255  ; float sigmax =  Size_fw; float sigmay =  Size_fw ; int cl_Q = (N_LB <30) ?2000 : 4000;/*100*(1+iii);*/ int n_bufs =m_nLabels;    Lmbd_trn = 0.5;
...
//////////////////////////////////////////////////////////////////
 	...
 	if(N_LB>30)Make_Gr_inf_pp(0.011,15,  7, 15);  else Make_Gr_inf(0,  7, 15);
	float Lm_tr[3] = {2.5*0.125, 1.25*0.125,1.*0.125};
	float ml[3] = {1./6, 1./6, 1./6};
	for(int q =0; q<3; q++){Smthnss[q].max = Lm_tr[q]; Smthnss[q].semi  =  Lm_tr[q]*ml[q];}
//////////////////////////////////////////////////////////
   for(int lr = 0; lr<2;lr++)
	 {
	     ...
	     int * rez =  (lr)? dR : dL;
		 BYTE * I_b = I_ims[lr];
		 copy_m_D( m_cost, lr);  int thr = 5; float alp =0.;
		 Make_Gr_fl_buf(  thr, lr,m_cost, N_LB, alp );
		/*if(Sc_out == 1)*/ K_mean_Flt_Add_new(I_b, sigma_c_mul, sigmax,sigmay, cl_Q, m_cost, n_bufs);
	    TRWS_CST(itr,Mrgnl, m_cost, rez,lr);
		 //this->min_cost(m_cost, rez);

		 if(!lr){
			 occ  = sigma_img_get(rez);      if (occ <= c_lbl) lr =2;  }
   }

  {
	   sigma_img_get(dL);

	if(occ > c_lbl)MkDspCorr_RL(  dL, dR);
	{if(occ > c_lbl)for(int i=0; i<1; i++)
	{
		int r_w = 4;  float cl_prc = sigma_img;//0.23;
	Gss_wnd_( r_w,  cl_prc, dL, 0, m_nLabels );
	Gss_wnd_( r_w,  cl_prc, dR, 1, m_nLabels );
	}}
	{if(occ > c_lbl)MkDspCorr_RL(  dL, dR);
	int r_w1 = 4;  float cl_prc1 = sigma_img;//0.23;
	Gss_wnd_( r_w1,  cl_prc1, dL, 0, m_nLabels );}
		...
		if(0)Intermed(dL, dR);
		int r = 4;//(N_LB >16) ? 4:2;
		if(occ > c_lbl) CorrectSub_new(dL, dR, /*intr_cst[0],*/ r, 1);
	else for(int i =0; i < m_nPixels; i++ ){ dL[i] *= 1;  }
   }
	//----------------------------------------------
	delete [] m_cost;
	delete [] Mrgnl;
...
void EDP::CorrectSub_new(int *outL, int *outR, /*double * m_cost,*/ int r,  int sc)
{
	...
	//double * cst = new double [N_PX*N_LB*r];
	...
	for(int i =0; i < 1; i++){
	srand( 27644437 );
	FOR_PX_p{ int rdl  = (abs(rand()))%dm - r/2;   rez[p] =  outL[p] + rdl; if (rez[p]<0) rez[p] = 0; if(rez[p]>=N_LB*r) rez[p] = N_LB*r -1;}
	int r_w = 11;  float cl_prc = (N_LB> 16)? ((N_LB>20)? 3.5 : 2.5):0.5;//0.23;
	Gss_wnd_ML( r_w,  cl_prc ,outL, rez, m_nLabels*r , r);
	FOR_PX_p {outL[p] = rez[p];}
	}
...
///// XX_TAU ///////////////////////////////
	...
	//---------------------------
	for(int i =0; i <m_width*m_nLabels; i++) tau_XL_d[i] = tau_XR_d[i] =-1;
	//----------------------------
...
//________________________________________
	...
	for(int t=2; t<Tau; t++)
	{
	 ...
	 if( xl-xr > 0) {dl= 0;    dr=1;}
	 else            { dl=1;     dr = (xr<m_nLabels-1) ? -xr : 2-m_nLabels; }
	 xl += dl;       xr += dr;  d = xl-xr;
	 XL_tau[t] = xl; XR_tau[t] = xr; tau_XL_d[ xl*m_nLabels + d]= t; tau_XR_d[ xr*m_nLabels + d]= t;
     }
...
{
////for(int c =0; c<3; c++ )
////for(int i =0; i<m_nPixels; i++) I_ims[2][i + m_nPixels*c] = (c<2)? ((c<1)? G_L_buf_x[i]*255: G_R_buf_x[i]*255): G_L_buf_y[i]*255;
//for(int i =0; i<m_nPixels; i++)m_answer[i] =Mask_RL[i]*7;
//int yy =150;
//for(int i =0; i<Tau; i++)
//{ int shf = m_height*m_width;
//	int x = XL_tau[i]; int y = XR_tau[i];
//	float vl = m_D_pnl[i+yy*Tau];
//	y = m_height - y-1; if(y<0) y= 0;
//     int vl1 = (int)vl; if(vl1>255) vl1=255;
//	 int vl2 = (int)(vl*8); if(vl2>255) vl2=255;
//	 int vl3 = (int)(vl*32); if(vl3>255) vl3=255;
//	for(int c =0; c<3; c++ )this->I_ims[2][x + y*m_width + shf*c] = (c<2) ? ((c<1)? vl1:vl2) : vl3;
//}
for(int i =0; i<m_nPixels*3; i++)I_ims[2][i]=100 + ((i/m_nPixels)%3)*30;

if(1)	{for(int i =0; i<Tau; i++)
{ int shf = m_height*m_width;
	int x = XL_tau[i];
	int y = x - XR_tau[i];
	x -= y/2;
...
{ int shf = m_height*m_width;
	...
	if(vlL<Cost_mean/2||vlR<Cost_mean/2){ if(vlL<Cost_mean/2){vl1 = 0; vl2=200; vl3=200;} if(vlR<Cost_mean/2){vl1 = 0; vl2=0; vl3=255;}
	for(int c =0; c<3; c++ )this->I_ims[2][x + y*m_width + shf*c] =(c<2) ? ((c<1)? vl3:vl2) : vl1;
	}
}
}
...
//{  int ste =0;
//if(Sltn_crv[WpH*yy+i].tau != Sltn_crv_tst[T_max_y[yy]-1-i].tau) ste=1;
//if(Sltn_crv[WpH*yy+i].d != Sltn_crv_tst[T_max_y[yy]-1-i].d) ste=1;
//if(Sltn_crv[WpH*yy+i].lb != Sltn_crv_tst[T_max_y[yy]-1-i].lb) ste=1;
//if(Sltn_crv[WpH*yy+i].rgb[0] != Sltn_crv_tst[T_max_y[yy]-1-i].rgb[0]) ste=1;
//if(Sltn_crv[WpH*yy+i].rgb[1] != Sltn_crv_tst[T_max_y[yy]-1-i].rgb[1]) ste=1;
//if(Sltn_crv[WpH*yy+i].rgb[2] != Sltn_crv_tst[T_max_y[yy]-1-i].rgb[2]) ste=1;
//if(ste)
//{
//	int a =0;
//}
...
//for(int c =0; c<3; c++ )
//{int shf = m_height*m_width;
//this->I_ims[2][i + j*m_width + shf*c] = 255;}
...
int y =x - XR_tau[Sltn_crv[WpH*yy+i].tau];
	...
	//int vl = m_D[i+yy*Tau];
	y = m_height - y-1; /*if(y<0) y= 0;*/
//int v1=(d>20)? (d-20)*12:0; int v2= (d>32)? (d-32)*9:0; int v3=(d>42)? (d-42)*12:0;
	int v1 = 0, v2 = 255, v3 = 0;
	//int v1=Sltn_crv[WpH*yy+i].rgb[0], v2=Sltn_crv[WpH*yy+i].rgb[1], v3=Sltn_crv[WpH*yy+i].rgb[2];
	//if(Sltn_crv[WpH*yy+i].lb%8 ==1){v1=255; v2=0; v3=0;}
	//if(Sltn_crv[WpH*yy+i].lb%8 ==2){v1=0; v2=0; v3=255;}
	//if(Sltn_crv[WpH*yy+i].lb%8 ==0){v1=0; v2=255; v3=0;}
	//if(Sltn_crv[WpH*yy+i].lb%8 ==3){v1=128; v2=0; v3=0;}
	//if(Sltn_crv[WpH*yy+i].lb%8  == 4){v1=0; v2=0; v3=128;}
 //    if(Sltn_crv[WpH*yy+i].lb/8 ==1){v1=255;  v2=255; v3=255;}
	for(int c =0; c<3; c++ )this->I_ims[2][x + y*m_width + shf*c] =(c<2) ? ((c<1)? v1:v2) : v3;
}
}

//int shf = m_width*m_height;
//for(int i =0; i<m_width; i++)
//for(int j=0; j<m_nLabels; j++)
//{
//            int tau  = tau_XL_d[j+ i*m_nLabels];
//			int vind =0, rind=0, lind=0;
//			if(!(tau<0))if(!(V_tau_frw[tau]<0)) vind=XR_tau[V_tau_frw[tau]]+ j*m_width;
//			if(!(tau<0))if(!(L_tau_frw[tau]<0)) lind= XR_tau[L_tau_frw[tau]]+ j*m_width;
//			if(!(tau<0))if(!(R_tau_frw[tau]<0)) rind=XR_tau[R_tau_frw[tau]]+ j*m_width;
//
//			for(int c =0; c<3; c++ )
//			{
//			unsigned char vl;
//			if(c==0) this->I_ims[2][vind+c*shf] =(i*32)+j*32;
//			if(c==1) this->I_ims[2][lind+c*shf] = (i*32) +j*32;
//			if(c==2) this->I_ims[2][rind+c*shf] =(i*32) +j*32;
//
//			}
//}
		...
		int truncDiffs)       // truncated differences
{
	...
	{//tau
		...
		int sumdfb[4] = {0,0,0,0};
				for (int b = 0; b < nColors; b++)
					   {
											...
											dfb[3]= abs(im1[yw + x] - (im2[yw+x2]- im2[yw+x2p])/2);
											 for(int q =0;q<4;q++)sumdfb[q] += (squaredDiffs )? dfb[q]*dfb[q]:dfb[q];
											}
										     else
											{
		    ...
		    // truncate diffs
			if (birchfield) {sumdiff =MIN(MIN(sumdfb[2], sumdfb[3]) , MIN(sumdfb[0], sumdfb[1]));}
		...
		float alph)       // truncated differences
{
	...
	{//tau
		...
		int sumdfb[4] = {0,0,0,0};
			...
			sum_ret += m_D[dsiIndex] += ( m_D[dsiIndex]+ dsiValue>maxsumdiff) ? maxsumdiff-m_D[dsiIndex]: dsiValue; dsiIndex++;
...
/*	float * c_D_pL = new float [Tau*m_height];
	float * c_D_pR = new float [Tau*m_height];*/ for (int t = 0; t <Tau*m_height; t++) {/*c_D_pR[t] = m_D_pR[t]; c_D_pL[t] = m_D_pL[t]; */c_D[t] = m_D[t]; }
	//----------------------------
	unsigned char rgb[3];
	float rgb_m[3] ={0,0,0};
	for(int i= 0; i< m_nPixels;i ++)
    ...
    double EDP::Cost_mean_vl()
{
 ...
 double min = sums[0]/cnt[0];  double max = sums[0]/cnt[0];
 for(int i =1;  i<m_nLabels; i++) {min = (min< sums[i]/cnt[i]) ? min: sums[i]/cnt[i]; max = (max> sums[i]/cnt[i]) ? max: sums[i]/cnt[i];}
...
{/////////////////// ALGORITHM //////////////////////////
for(int p= 0; p<m_height*Tau; p++)Mrg[p]= m_D[p];
...
for(int d = 0; d< d_mx; d++)mrg[xld[d]] -=mrg_min;
...
for(int x = 0; x < m_width; x++)
{
	...
	if(vl < 0) for(int c = 0; c < 3; c++)I_ims[2][x + l*m_width + c*N_PX] = (c==2) ? 255: 64;
	else         for(int c = 0; c < 3; c++)I_ims[2][x + l*m_width + c*N_PX] = (vl >255) ? 255: (int)vl;
	}
...
int i0 = x + st*m_width;
int d =0; sum +=  vw[x] = min = cst[i0];
for(int l =1; l < N_LB; l++){sum += vw[x + l*m_width] = cst[i0 + l*N_PX]; if(min > cst[i0 + l*N_PX]){d =l; min = cst[i0 + l*N_PX];}}
vw[x + d*m_width] = -1;
...
for(int x =0; x<m_width; x ++)
{
 ...
 {  float dl =(mrg[xld[d]]  - mrg_min);  avmin += dl; cnt ++;}


}// end dir ////////////////
avmin /= cnt;  //thr = thr*avmin;
thr = thr*(avmin - mm)*(avmin - mm); if(thr> 2.5) thr *=0.91;
for(int y =0; y<m_height; y++)
for(int x =0; x<m_width; x ++)
{
 int d_mrg;
 ...
 float *mrg = &mm_D(y);
 int *xld = &XLD(x);
 for(int d = 0; d< d_mx; d++){ float arg =  ((mrg[xld[d]] - mm)/mm);
...
if(!dr)for(int y =0; y<m_height; y++)
for(int x =m_width-1; x>=0; x --)
{
 int d_mx = (x + 1 <m_nLabels) ? x +1 : m_nLabels;
...
//int sig =1 ; float *div =  new float [m_nPixels];  for(int p =0; p<m_nPixels; p++)div[p]=1; GaussCosConv2DFst(sig, m_width,m_height, div);
//for(int l = 0; l <m_nLabels; l++)
//{GaussCosConv2DFst(sig, m_width,m_height, &m_cost[m_nPixels*l]);
//for(int p =0; p<m_nPixels; p++)m_cost[m_nPixels*l+ p] /=div[p];}
//delete [] div;
}
...
for(int x =m_width-1; x>=0; x --)
{
 ...
 int *xld = &XLD(x);
for(int d = 0; d< m_nLabels; d++)
if(d< d_mx)ret += m_D[y*Tau+xld[d]] = m_cost[y*m_width+x + d*m_nPixels];
}
else for(int y =0; y<m_height; y++)
for(int x =0 ; x< m_width; x ++)
{
...
delete [] supp;
	 ...
	 {
		 int H[1000]; for(int i =0; i <1000; i++) H[i]=0;
		for(int i =0; i < m_nPixels; i++ )
	   {
	   int x = i%m_width; int y = i/m_width;
     ...
     int dx = 0; for(int c=0;c <3; c++)dx += abs(I_ims[0][i + c*m_nPixels] - I_ims[0][ixpp + c*m_nPixels]);
	 int dy = 0; for(int c=0;c <3; c++)dy += abs(I_ims[0][i + c*m_nPixels] - I_ims[0][iypp + c*m_nPixels]);
	 H[dx]++; H[dy]++;
	 }
	int first = m_nPixels*2/3; int scnd = m_nPixels*4/3;
	int th1 = -1, th2 = -1; int sum = 0;
	for(int i =0; i <1000; i++){sum += H[i]; if(th1 < 0) {if(sum >= first) th1 =i; } if(th2 < 0) {if(sum >= scnd){ th2 =i; i=1001;}} }
...
{
	BYTE * rezb = new BYTE [m_nPixels*3]; for(int p =0; p <3*m_nPixels; p++){rezb[p] = rez[p%N_PX];}
	BYTE * ol_msk = new BYTE [N_PX]; FOR_PX_p {ol_msk[p] =0;}
//===
	WMask = new int [m_nPixels];
	ResInd   = new POINT [m_nPixels];

	//--------------------------------
	GetClrMask(thr, rezb); float cof = thr_ar* N_PX; int cnt = 0;
	FOR_PX_p { int x = p%m_width;
...
float *gss_r_wt = new float [m_width+m_height];
	...
	for(int i=0; i<1000; i++ )gss_c_wt[i]= exp(-(i*i)/(2.*cl_fc*cl_fc));
	//----------------------------------
    cnt =3; int st =1; while (st&&cnt){ st = 0; cnt--;
	FOR_PX_p{
	if(!ol_msk[p])
	{    int out = mask_BF_med(p, ol_msk, rez, r_w, gss_c_wt,  gss_r_wt, lr);
	if(out <0) {
	 st = 1;  }
	else {rez[p] = out; ol_msk[p] =1;}
	}}
    ;
	}//------------------------------------
	delete [] gss_r_wt;
...
float cst = 0;// PCst_mlt/(d_x- d_xg);
 for(int d = d_x; d>=d_xg; d--) pcst[xld[d]] *= cst;
 }
 }
 else
{
...
void EDP:: MkDspCorr_RL(int *inL, int *inR)
{
	...
	int* outL = new int [m_nPixels];for(int y =0; y<m_nPixels; y++)outL[y] = inL[y];
	int* outR = new int [m_nPixels];for(int y =0; y<m_nPixels; y++)outR[y] = inR[y];
...
int ind = x + y*m_width;
int dr = inR[ind];
int indL =(x+dr<m_width) ? x + dr + y*m_width: m_width-1 + y*m_width;

if(x+dr>m_nLabels && x+dr<m_width){int dl = inL[indL];  outR[ind] = (dl <dr) ? dl:dr;}
else outR[ind] = dr;
...
if(m_width-1 - x +dl >m_nLabels && x - dl >=0 ) {int dr = inR[indR]; outL[ind] = (dr <dl) ? dr:dl;}
else outL[ind] = dl;
}
...
for(int d = 0; d< d_mx; d++) mrg[xrd[d]] *= mul;
}
else
for(int y =0; y<m_height; y++)
for(int x =0; x<m_width; x ++)
{
  ...
  for(int d = 0; d< d_mx; d++) mrg[xld[d]] *= mul;
...
void   EDP::Gss_wnd_ML( int r_w, float cl_fc, int * sup,  int * DspM, int o_s, int rg )
{
     ...
     float * cst = new float [N_PX]; float sigcst = 16;
	 	FOR_PX_p {  /*I_ims[2][p + 2*N_PX] = I_ims[2][p + N_PX] = I_ims[2][p] = */cst[p] = (exp(-cst_sub_pix(  (float)(DspM[p])/rg, p, I_ims[0], I_ims[1])/sigcst));  }
	///////////////////////////////////////////////
	int dm = 2*r_w+1;
	int size_w = dm*dm;
	int * rez = new int [m_nPixels];
	...
	for(int i=0; i<m_width+m_height; i++ )gss_r_wt[i]= exp(-(i*i)/(2.*sgm_r*sgm_r));
	for(int i=0; i<1000; i++ )gss_c_wt[i] =/* (i> 2*cl_fc)? 0 : */exp(-(i*i)/(2.*cl_fc*cl_fc));
...
void   EDP::Gss_wnd_( int r_w, float cl_prc, int * DspM, int iLR, int o_s )
{
	...
	for(int i=0; i<m_nPixels; i++){ int min,max;
	    Histo_min_max(i, &min, &max, DspM,  r_w,  Histo);
        if(min!=max)rez[i]=gss_wnd_( i, min, max,  DspM, r_w,gss_c_wt, gss_r_wt, iLR, Histo);
		else rez[i] = min;
	}
...
void   EDP::med_fl( int r_w, int r_c,  unsigned char* inb)
{
   ...
   {
	int xx=  x +i;     if(xx<0)  xx = 0;       if(xx>=m_width)  xx = m_width-1;
	int yy=  y +j;     if(yy<0)  yy = 0;       if(yy>=m_height) yy = m_height-1;
	int ind_t = xx + yy*m_width;
    for(int c = 0; c <3; c++) hs[inp*3 +c] = inb[ind_t + c*m_nPixels];
   inp++;
   }
		for(int j= 0; j < np; j++)
		              {  w[j] = 0;
			 for(int c = 0; c <3; c++) w[j] +=(hs[np/2*3+c] - hs[j*3+c])*(hs[np/2*3+c] - hs[j*3+c]);
						  w[j]  = exp(-w[j]/r_c);
		             }
        for(int i =0; i < np; i++)
		             { dsts[i] = 0;
		              ...
		              { float dst = 0;
			 for(int c = 0; c <3; c++) dst +=(hs[i*3+c] - hs[j*3+c])*(hs[i*3+c] - hs[j*3+c])*w[j];
		      dsts[i] += sqrt(dst);
		             }}
		float min = dsts[0]; int i_min =0;  for(int i =1; i < np; i++) if(min > dsts[i]){min = dsts[i]; i_min = i;};
		for(int c = 0; c <3; c++)out[x+ y*m_width + c*m_nPixels] = hs[i_min*3+c];
	}

///////////////////////////////////////////////////////
for(int i= 0; i< m_nPixels*3;i ++)  inb[i] = out[i];
delete [] w;
delete [] out;
delete [] hs;
delete [] dsts;
...
for(int j=-r_w; j<=r_w; j++)
{
	...
	{int l = buf0[ind_t]; max[0] = (max[0]> l ) ? max[0]:l;  min[0] = (min[0]< l) ? min[0]:l;}
}
for(int i=min[0]; i <= max[0]; i++){ Histo[i]=0;}
}
...
for(int j=-r_w; j<=r_w; j++)
{
	...
	int y= jg+j;     if(y<0) y=0; if(y>=m_height) y = m_height-1;  int ind_t = x+y*m_width;
	int l = buf0[ind_t];
	if(l!=Z){max[0] = (max[0]> l ) ? max[0]:l;  min[0] = (min[0]< l) ? min[0]:l; st =1;}

}
if(!st){max[0] = Z; min[0] = Z; return;}
for(int i=min[0]; i <= max[0]; i++){ Histo[i]=0;}
}
...
for(int j=-r_w; j<=r_w; j++)
{
	int x=  ig+i; if(x<0) x=0; if(x>=m_width) x = m_width-1;
	int y= jg+j;     if(y<0) y=0; if(y>=m_height) y = m_height-1;  int ind_t = x+y*m_width;
	{int l = buf0[ind_t]; /*int st = map[ind_t];*/ /*if(!st)*/{max[0] = (max[0]> l ) ? max[0]:l;  min[0] = (min[0]< l) ? min[0]:l;} }
}
if(min[0] >max[0]) min[0] = max[0] = -1;
if(min[0]!=-1)for(int i=min[0]; i <= max[0]; i++){ Histo[i]=0;}
}
...
//float maxh = Histo[min];int med_ind =min;
//for(int i=min+1; i <= max; i++){ if(maxh<Histo[i]){maxh = Histo[i];med_ind =i;}}
//return med_ind;
for(int i=min; i <= max; i++){ sum +=Histo[i];}
...
if((x1_1==x1_0))                     { int ind1 = y*m_width+x1_0;
    out_buf [ii] = 0;
	int ind0_ = ind0;
    for(int ic=0; ic<6; ic++, ind0_ += m_nPixels, ind1 += m_nPixels) out_buf [ii] +=abs(buf0[ind0_]-buf1[ind1]);
	out_buf [ii] = ( out_buf [ii] < 3*chTr ) ? out_buf [ii] : 3*chTr;}
else                                            {
indp[0] = x1_0 + y*m_width;
indp[1] = (x1_0+1<m_width) ? x1_0+1 + y*m_width: m_width-1  + y*m_width;
...
{
REAL * ms[4];
for(int i=0;i<4;i++) {ms[i]=new REAL [m_nPixels*Lbs_S];  memset( ms[i], 0, m_nPixels*Lbs_S*sizeof(REAL));}
//-------------------------------------------------------------------
 dsp_sp_mrf( iti, ms);
//-------------------------------------------------------------------
...
/////////////////////////////////////////////////
	...
	buf_out[i+ImgSh]= dx*8;}//(dy+(-dx1+dy1)*mul_sq2)*4;}
...
{
	int ret = 0; float min = buf_lb[p]; for(int i =1; i < N; i ++) if(min > buf_lb[p + i*m_nPixels]){min = buf_lb[p + i*m_nPixels]; ret =i;}
	return ret;
}
void EDP::ROTH_lb(int n_lb, float * buf_lb, unsigned char * I_b, int * answer)
{
	...
	double  * buf2srt = new double[size];  for(int p = 0; p < size; p++)buf2srt[p] = 0;
		 ...
		 float sum =0; for(int p = 0; p < size; p++) {sum += buf2srt[p]; }
		 buf2srt[0] = -log(buf2srt[0]/sum + logmin); float min = buf2srt[0];
...
void EDP::ROTH_lb(int n_lb, float * buf_lb, unsigned char * I_b)
{
	...
	int rule_cnst =3; int r = 32;  int div = 256/r; int size = r*r*r;  POINT p_lb0 = {n_lb%rule_cnst, n_lb/rule_cnst};
	 POINT_F p_l = {p_lb0.x*(m_width/rule_cnst) + m_width/rule_cnst/2, p_lb0.y*(m_height/rule_cnst) + m_height/rule_cnst/2};
...
{
	int *i_exp2 = new int [31]; i_exp2[0] = 1; for(int i = 1; i< 31; i++ ) i_exp2[i] = 2*i_exp2[i-1];
	int st =1; int N  = 0; while(st){ if(n_p <= i_exp2[N]) st = 0; else N++; }
   int size= i_exp2[N];
   int *buf_max[2];
   int * buf_ind[2];
   ...
   for(int i = 0 ; i < size; i++){buf_ind[0][i] = i; if(i<n_p) buf_max[0][i] = buf_to_sort[i]; else buf_max[0][i] = 0;}
////////////////////////////////////////////////////////////

st=0;
for(int in=1; in <= N ; in ++ )
{   SetSort_n(i_exp2, in, N,  buf_max[(st+1)%2],  buf_ind[(st+1)%2],  buf_max[st],  buf_ind[st]); st=(st+1)%2;  }
for(int i = 0 ; i < n_p; i++){ind_out[i] = buf_ind[st][i]; }
///////////////////////////////////////////
delete [] buf_max[0];
delete [] buf_max[1];
delete [] buf_ind[0];
...
{
	for( int ci= 0; ci<3; ci++)mn[ci]+=bufb[i+ci*m_nPixels];
}
for( int ci= 0; ci<3; ci++)mn[ci]/=m_nPixels;
float sig_b=0;
for( int i= 0; i<size; i++)
{
	for( int ci= 0; ci<3; ci++)sig_b+=(mn[ci]-bufb[i+ci*m_nPixels])*(mn[ci]-bufb[i+ci*m_nPixels]);
}
...
for( int i= 0; i<size; i++)
{
	{for( int ci= 0; ci<6; ci++)sig_f+=(buf[i+ci*m_nPixels])*(buf[i+ci*m_nPixels]);}
}
sig_f=sqrt(sig_f/m_nPixels); float ret = sig_b/sig_f;
sigma_img = 1./sigma_img; sigma_img *= sig_f;
return ret;
}
...
double    EDP:: sigma_img_get( int *dL)
	{
	float mean =0;  double ret;     for(int p =0; p < m_nPixels; p++) mean += dL[p]; mean /= m_nPixels;
    sigma_img = 0;      for(int p =0; p < m_nPixels; p++) sigma_img += (mean-dL[p])*(mean - dL[p]);  ret = sigma_img /= m_nPixels; sigma_img =9.* (sigma_img)/m_nLabels/m_nLabels;
	return ret;
}
...
{   int i = cr_in[p].x + dxdy[k].x; int j = cr_in[p].y + dxdy[k].y;
	POINT ij = {i,j};
	if(i>=0&&i<ij_img.x &&j>=0&&j<ij_img.y)
	{//chk_nbrs
      ...
      //-----------------------------------------------
	   int t_vl =0; /**/if(st_msk) for(int c=0; c<3; c++, i_0 += ImgSh, i_sh += ImgSh)  /**/ t_vl += abs(FillBf[i_0]-FillBf[i_sh]);
		//---------------------------------------------
		if(t_vl<=thr&&st_msk)
...
int prFl=1;
//===============
	while((prFl=FillStepClr(prFl, thr, cr_in_out[(s+1)%2],cr_in_out[s], FillBf, ij_img))){s=(s+1)%2;}
//===============
...
void EDP::GetClrMask(int thr, BYTE* FillBf)
{
		 ...
		 POINT *cr_in_out[2];
///////////////////////////////
for(int i=0;i<2;i++){cr_in_out[i] =new POINT [size];}
...
////////////////////////////////
   for(int i=0;i<size;i++)
  {
	p.x=i%ij_img.x;
	p.y=i/ij_img.x;
	if((!WMask[i]))FillFrPoinClr(thr, p, cr_in_out, FillBf, ij_img);
   }
   IndCount--;
///////////////////////////////////////
for(int i=0;i<2;i++){delete []  cr_in_out[i];}
...
void EDP:: Make_Gr_fl( int thr, int thr2, int lr)
{
 ...
 for(int i =0; i < m_nPixels; i++ )
	   ...
	   int iypp = ypp*m_width + x;
       int ixpp = y*m_width + xpp;
	   dgr2[i].x = (WMask[i] - WMask[ixpp]) ? 1 : 0; dgr2[i].y = (WMask[i] - WMask[iypp]) ? 1 : 0;
	   }*/
 ...
 /* for(int i =0; i < m_nPixels; i++ )
	   ...
	   int dxpp = rez[ixpp];
	   int dypp = rez[iypp];
	   if(d == dxpp){dcst[i].x = fabs(cst[i + d*m_nPixels] - cst[ixpp + d*m_nPixels]); if(dcst[i].x > 255) dcst[i].x =255;}
	   else {int r1 = fabs(cst[i + d*m_nPixels] - cst[ixpp + d*m_nPixels])*255; if( r1> 255) r1 =255;
	            int r2 = fabs(cst[i + dxpp*m_nPixels] - cst[ixpp + dxpp*m_nPixels])*255; if( r2> 255) r2 =255;
                dcst[i].x = (r1+r2)/2;
	          }
	   if(d == dypp){dcst[i].y = fabs(cst[i + d*m_nPixels] - cst[iypp + d*m_nPixels])*255; if(dcst[i].y > 255) dcst[i].y =255;}
	   else {int r1 = fabs(cst[i + d*m_nPixels] - cst[iypp + d*m_nPixels])*255; if( r1> 255) r1 =255;
	            int r2 = fabs(cst[i + dypp*m_nPixels] - cst[iypp + dypp*m_nPixels])*255; if( r2> 255) r2 =255;
                dcst[i].y = (r1+r2)/2;
	          }
	   }
  float ml =0, mh = 0; int cml =0, cmh =0;for(int i =0; i < m_nPixels; i++ )
  {if(dgr[i].x){mh += dcst[i].x; cmh++;} else{ ml += dcst[i].x; cml ++; }
  if(dgr[i].y){mh += dcst[i].y; cmh++;} else{ ml += dcst[i].y; cml ++; } }
  ml /= cml*5; mh /= cmh*5;
*/
 for(int i =0; i < m_nPixels; i++ )
	   {
	   ...
	   int iymm = ymm*m_width + x;
       int ixmm = y*m_width + xmm;
	   if(msk_grp[i].x && dgr[i].x && ResInd[WMask[i]].x > thr2) { msk_grp[i].x = 0; msk_grm[ixpp].x = 0;}
       if(msk_grp[i].y && dgr[i].y && ResInd[WMask[i]].x > thr2) { msk_grp[i].y = 0; msk_grm[ixpp].y = 0;}
 ...
 //     	  msk_grp[i].x = 0;
	...
	//	  msk_grp[i].y = 0;
	//      msk_grm[iypp].y = 0;
	//  if(dgr[i].x) {msk_grp[i].x = 1;  msk_grm[ixpp].x = 1;}
	//  if(dgr2[i].x)  {msk_grp[i].x = 2;  msk_grm[ixpp].x = 2;}
	// //
	//  if(dgr[i].y ) {msk_grp[i].y = 1;  msk_grm[iypp].y = 1;}
	//  if(dgr2[i].y )  {msk_grp[i].y = 2;  msk_grm[iypp].y = 2;}
	//

 //}


 //for(int p = 0; p < m_nPixels; p++){ I_ims[2][p] = msk_grp[p].x*127;
 //                                                              I_ims[2][p + m_nPixels] = I_ims[2][p + 2*m_nPixels] = msk_grp[p].y*127; }
delete [] dgr;
//delete [] dgr2;
delete [] WMask;
delete [] ResInd;
...
void EDP:: Make_Gr_fl_buf( int thr, int lr, double *cost, int L, float alp )
{
	...
	{
	int ind = WMask[p];
	mvl[ind] += test[p]; cnt[ind]++;
	}
	FOR_I(IndCount) mvl[i] /= cnt[i];
	FOR_PX_p {tb[p] =/* (ResInd[WMask[p]+1].x  > 0.005*N_PX) ? */tb[p]*(1-alp) + mvl[WMask[p]]*alp /*:tb[p]*/;}
	}//-----------------
