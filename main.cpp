...
uchar* Img_scl_s(uchar* in, int w, int h, int sc)
{
	...
	for (int i = 0; i < ws; i++)
		for (int j = 0; j < hs; j++)
		{
			for (int c = 0; c < 3; c++)out[i + j * ws + c * ws * hs] = in[i * sc + j * sc * w + c * w * h];
		}
		...
		for (int j = 0; j < h; j++)
		{
			...
			for (int c = 0; c < 3; c++)
			{
				...
				for (int pi = 0; pi < 4; pi++) {
					rc[pi] = (float)inim[p[pi] + c * sh_in] - vloutim; rc[pi] *= rc[pi];
					wtc[pi] += rc[pi];
				}
			}
			for (int pi = 0; pi < 4; pi++) wtc[pi] = exp(-wtc[pi] / 2 / sig);
			float wtsum = 0; out[i + j * w] = 0;
			for (int pi = 0; pi < 4; pi++)
			{
...
{
	float m1[3] = { 0,0,0 }; float d1[3] = { 0,0,0 }; float m2[3] = { 0,0,0 }; float d2[3] = { 0, 0, 0 };
	...
	for (int c = 0; c < 3; c++) { m1[c] /= cnt; m2[c] /= cnt; }
	for (int p = 0; p < cnt * 3; p++)
	{
		int cl = p / cnt;
	...
	float thr = 0.16;
	int steq = 0;
	for (int c = 0; c < 3; c++) { d1[c] = sqrt(d1[c] / cnt); d2[c] = sqrt(d2[c] / cnt); }
	for (int c = 0; c < 3; c++) {
		float vlm, vld;
	...
	if (steq)
	{
		for (int c = 0; c < 3; c++)
		{
			...
			// float m11 = MN_get(&I1[cnt*c],  cnt ); float m22 = MN_get(&I2[cnt*c],  cnt );
			 //float d11 = STD_get(&I1[cnt*c], m11,  cnt ); float d22 = STD_get(&I2[cnt*c], m22, cnt );
			// m11 =m11;
		}
...
/*    const int32_t dims[3] = {width,height,width}; */// bytes per line = width
	...
	uchar* Images[2]; for (int i = 0; i < 2; i++) Images[i] = new uchar[width * height * 3];
	int Lt = Img_set(Images[0], Images[1], I1->data, I2->data, width * height);
	Lt = 1;
	int scf = 1;
	int med_f = 0;

	uchar* Imsc[2] = { NULL, NULL };
	uchar* sc_msk;
